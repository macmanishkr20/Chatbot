...
EY's MENA fiscal year runs **July 1 → June 30** (EY global convention).
FY26 covers Jul 2025 – Jun 2026.

Override via the ``FISCAL_YEAR_START_MONTH`` env var (1-12) if a different
convention is needed for testing or another business unit.
"""
...
def fiscal_year_range(fy: str) -> Tuple[date, date]:
    """Inclusive (start, end) for a fiscal-year label.
...
def fiscal_quarter_range(fy: str, fq: str) -> Tuple[date, date]:
    """Inclusive (start, end) for a fiscal quarter inside a fiscal year."""
    fq = fq.strip().upper()
