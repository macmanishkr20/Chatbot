...
export class LmsFormCardComponent {
  ...
  /** Existing submission result (so the message renders the outcome on reload). */
  result = input<FormSubmitResult | null | undefined>(null);

  /** Emitted with the submission outcome so the host can persist it. */
  ...
  /** Capture <input>/<select>/<textarea> events and forward to the field state. */
